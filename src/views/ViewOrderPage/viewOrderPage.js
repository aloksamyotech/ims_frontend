...
const InvoicePage = () => {
  ...
  const updateOrderStatus = async (id, action) => {
    const updatedOrder = { _id: id, action: action };
    ...
    try {
      ...
      if (response.success === true) {
        ...
        setDialogMessage(`Order ${action === 'approve' ? 'approved' : 'cancelled'} successfully!`);
        setDialogAction(action);
        setOpenDialog(true);
      } else {
        setDialogMessage(`Failed to ${action === 'approve' ? 'approve' : 'cancel'} order`);
      ...
      <Dialog open={openDialog} onClose={handleCloseDialog} PaperProps={{ sx: { width: 350, height: 250 } }}>
        <DialogTitle sx={{ textAlign: 'center' }}>
          <Box display="flex" flexDirection="column" alignItems="center" gap={2}>
            ...
            <Typography variant="h4" sx={{ fontSize: '1.8rem', marginBottom: 1 }} color={dialogAction === 'approve' ? '#28a745' : 'red'}>
              {dialogMessage}
            </Typography>
          </Box>
        </DialogTitle>
